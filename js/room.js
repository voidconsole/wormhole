...
export async function generateInviteLink(password, roomId) {
	...
	// Build URL relative to current path so it works whether the app is
	// at the root (voidconsole.github.io/) or a subdirectory (/wormhole/).
	const { origin, pathname } = window.location;
	const dir = pathname.substring(0, pathname.lastIndexOf("/") + 1);
	const baseUrl = origin + dir + "chat.html";

	// encodeURIComponent ensures +, /, = in base64 are safely transported
	return `${baseUrl}?roomId=${roomId}&key=${encodeURIComponent(keyB64)}`;
}
...
export function parseInviteLink() {
	...
	const key = params.get("key"); // URLSearchParams already URL-decodes this
	if (roomId && key) return { roomId, keyB64: key };
	return null;
}
